...
def read_json_line(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            output.append(json.loads(line, encoding='utf-8'))
...
def get_multitask_instances_for_valid_tasks(task_instances, tag_statistics, test_mode=False):
    ...
    for subtask in task_instances.keys():
        ...
        if not test_mode:
            if len(current_question_tag_statistics) > 1 and current_question_tag_statistics[
                1] >= MIN_POS_SAMPLES_THRESHOLD:
    ...
    for subtask in subtasks:
        ...
        if test_mode:
            for id, text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet, tokenized_tweet_with_masked_chunk, gold_chunk, label in \
                    task_instances[subtask]:
                instance = (id, text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet,
            ...
            for text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet, tokenized_tweet_with_masked_chunk, gold_chunk, label in \
                    task_instances[subtask]:
                instance = (text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet,
                            tokenized_tweet_with_masked_chunk)
                if text not in text_to_subtask_instances:
                    original_text_list.append(text)
                    text_to_subtask_instances[text] = dict()
    ...
    print('orig_len', len(original_text_list))
...
def split_multitask_instances_in_train_dev_test(multitask_instances, TRAIN_RATIO=0.6, DEV_RATIO=0.15, test_mode=False):
    ...
    if test_mode:
        for id, tweet, _, _, _, _, _, _, _ in multitask_instances:
...
def get_raw_scores(data, prediction_scores, positive_only=False):
    ...
    # text :: candidate_chunk :: candidate_chunk_id :: chunk_start_text_id :: chunk_end_text_id :: tokenized_tweet :: tokenized_tweet_with_masked_q_token :: tagged_chunks :: question_label
    for (text, chunk, chunk_id, chunk_start_text_id, chunk_end_text_id, tokenized_tweet,
         tokenized_tweet_with_masked_chunk, gold_chunk, label), prediction_score in zip(data, prediction_scores):
        original_text = text
        # print(text)
        # print(chunk)
        # print(original_text)
