...
import pickle
import logging
from model.utils import log_list, print_list, save_in_pickle
...
def make_instances_from_dataset(dataset, annotation):
    ...
    for annotated_data in dataset:
        ...
        candidate_chunks_from_tokens = [' '.join(tweet_tokens[c[0]:c[1]]) for c in
        ...
        """
        # Update the list
        candidate_chunks_from_text = [e for ignore_flag, e in zip(ignore_flags, candidate_chunks_from_text) if
                                      not ignore_flag]
        candidate_chunks_from_tokens = [e for ignore_flag, e in zip(ignore_flags, candidate_chunks_from_tokens) if
                                        not ignore_flag]
        candidate_chunks_offsets = [e for ignore_flag, e in zip(ignore_flags, candidate_chunks_offsets) if
                                    not ignore_flag]
        candidate_chunks_offsets_from_tweet_tokens = [e for ignore_flag, e in
                                                      zip(ignore_flags, candidate_chunks_offsets_from_tweet_tokens) if
                                                      not ignore_flag]
        ...
        final_tweet_tokens = [URL_TOKEN if e.startswith("http") or 'twitter.com' in e or e.startswith('www.') else e for
                              e in tweet_tokens]
        final_candidate_chunks_with_token_id = [(f"{c[0]}_{c[1]}", ' '.join(tweet_tokens[c[0]:c[1]]), c) for c in
                                                candidate_chunks_offsets_from_tweet_tokens]
        ...
        for question_tag, question_key in question_keys_and_tags:
            ...
            for candidate_chunk_with_id in final_candidate_chunks_with_token_id:
                ...
                if question_tag in ["relation", "gender_male", "gender_female", "believe", "binary-relation",
                                    "binary-symptoms", "symptoms", "opinion"]:
                    # If the question is a yes/no question. It is for the name candidate chunk
                    special_tagged_chunks = get_tagged_label_for_key_from_annotation(question_key,
                    ...
                    question_label, tagged_chunks = get_label_for_key_from_annotation(question_key,
                                                                                      annotation_tweet_tokens,
                                                                                      candidate_chunk)
                ...
                task_instances_dict[question_tag].append((id,
                    text, candidate_chunk, candidate_chunk_id, chunk_start_text_id,
                    chunk_end_text_id, tokenized_tweet, ' '.join(
                        final_tweet_tokens[:chunk_start_id] + [
                            Q_TOKEN] + final_tweet_tokens[chunk_end_id:]),
...
def main(data_file, save_file, annotations):
    ...
    task_instances_dict, tag_statistics, question_keys_and_tags = make_instances_from_dataset(dataset, annotations)
    # print(task_instances_dict)
    # print(tag_statistics)
    # Save in pickle file
    logging.info(f"Saving all the instances, statistics and labels in {save_file}")
    # print(task_instances_dict, tag_statistics, question_keys_and_tags)
...
if __name__ == '__main__':
    ...
    pos_ano = {"part1.Response": ["yes"], "part2-age.Response": ["Not Specified"],
               "part2-close_contact.Response": ["Not Specified"], "part2-employer.Response": ["Not Specified"],
               "part2-gender.Response": ["Not Specified"], "part2-name.Response": [["Not Specified"]],
               "part2-recent_travel.Response": ["Not Specified"], "part2-relation.Response": ["Not Specified"],
               "part2-when.Response": ["Not Specified"], "part2-where.Response": ["Not Specified"]}
    neg_anot = {"part1.Response": ["yes"], "part2-age.Response": ["Not Specified"],
                "part2-close_contact.Response": ["Not Specified"], "part2-gender.Response": ["Not Specified"],
                "part2-name.Response": [["Not Specified"]],
                "part2-relation.Response": ["No"], "part2-when.Response": ["Not Specified"],
                "part2-where.Response": ["Not Specified"]}
    can_not_anot = {"part1.Response": ["yes"], "part2-relation.Response": ["Yes"],
                    "part2-symptoms.Response": ["Not Specified"], "part2-name.Response": [["Not Specified"]],
                    "part2-when.Response": ["Not Specified"], "part2-where.Response": ["Not Specified"]}
    death_anot = {"part1.Response": ["yes"], "part2-age.Response": ["Not Specified"],
                  "part2-name.Response": ["Not Specified"],
                  "part2-relation.Response": ["Not Specified"],
                  "part2-when.Response": ["Not Specified"], "part2-where.Response": ["Not Specified"]}
    cure_anot = {"part1.Response": ["no"], "part2-opinion.Response": ["no_cure"],
                 "part2-what_cure.Response": ["Not Specified"], "part2-who_cure.Response": ["Not Specified"]}
    anots = [pos_ano, neg_anot, can_not_anot, death_anot, cure_anot]
    for part, ano in zip(file_names, anots):
        main('data/test_data/shared_task-test-' + part + '.jsonl', 'data/test_data/' + part + '.pkl', ano)
